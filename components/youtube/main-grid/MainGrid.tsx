"use client";
// Main grid scene — production export for app/page.tsx.
// Assembled by copying inline MainGridView + MainGridGinnView + all transitive
// deps (cards, modals, chrome, helpers, constants) verbatim from
// stories/youtube/FullStory.stories.tsx (2026-04-21 Vercel migration).
//
// Navigation: linkTo() from @storybook/addon-links replaced with React context
// so the scene works in both Storybook (wrapper provides adapter) and Next.js
// (app/page.tsx provides router.push-based handler).
// Asset paths use /sounds/... /sphere-loop.mp4 /media/... /avatars/... (absolute).

import * as React from "react";
import { useMediaQuery } from "@/components/use-media-query";
import { MOCK_STATS } from "../../../stories/youtube/mock-data";
...
import type { PageNavProps, PageTarget } from "@/components/youtube/pages/nav";

// Nav context: PageFooter (defined inline below) reads this to replace linkTo calls.
// MainGrid wraps its children with a Provider that passes through onNavigate.
const NavContext = React.createContext<((target: PageTarget) => void) | undefined>(undefined);

// Shim: preserves the original `linkTo("...", "...")` call signature used inside
// PageFooter + other orphan views. In production, returns a click handler that
// calls the context nav handler with the matching PageTarget.
function linkTo(_kind: string, name: string): () => void {
  // Map "Interview Agents" etc. to PageTarget.
  const targetMap: Record<string, PageTarget> = {
  ...
  const target = targetMap[name] ?? "visualization";
  return () => {
    // Read nav handler at click time (not bind time) so late-mounted providers still work
    const handler = _currentNavHandler;
    handler?.(target);
  };
}
// Module-scope latch; MainGrid sets this each render so linkTo() captures the active handler.
let _currentNavHandler: ((target: PageTarget) => void) | undefined;
...
// ─── Public export ───────────────────────────────────────────────────────────
export function MainGrid({ orangeColor = "#E14920", onNavigate }: { orangeColor?: string } & PageNavProps) {
  // Latch onNavigate so the linkTo shim (used inline by PageFooter) can reach it.
  _currentNavHandler = onNavigate;
  return (
